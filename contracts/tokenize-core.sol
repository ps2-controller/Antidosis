...
interface ERC721TokenReceiver {
	...
	function onERC721Received(address _operator, address _from, uint256 _tokenId, bytes _data) external returns(bytes4){
		(address memory _distributionAddress, 
			...
			bytes memory _deploymentData) = abi.decode(_data, (
				...
				uint,
				bytes));
		require(lock721Token(_operator, _distributionAddress, _paymentAddress, _tokenId, _erc20Supply, _erc20Name, _erc20Symbol, _erc20Decimals, _erc20DeploymentAddress, _minimumShares, _deploymentData), "Error receiving token");
		...
		bytes _deploymentData) 
	{
		...
		AssetTokenizationContract newAssetTokenizationContract = new AssetTokenizationContract(_tokenToLockAddress, _distributionAddress, _paymentAddress,	 _tokenToLockId, _erc20Supply, _erc20Name, _erc20Symbol, _erc20Decimals, _minimumShares, _deploymentData);
